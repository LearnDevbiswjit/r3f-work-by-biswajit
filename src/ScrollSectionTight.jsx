  ...
  return (
    <div style={{ height: '100vh', overflow: 'hidden' }}>
      ...
      >
        ...
        <ScrollControls pages={pages} distance={3} damping={0.50}>
...
function Scene ({ sheet, guiFadeDefaults = {} }) {
  ...
  useFrame((state, delta) => {
    ...
    const contRadial = new THREE.Vector3(contLocal.x, 0, contLocal.z).normalize()
    ...
    const cameraLocalOffset = new THREE.Vector3(
      ...
      cameraUpOffset + sign * ((brickSpec.width / 2) * pathScale + cameraSideOffset),
      0
    )
    ...
    const contXAxis = new THREE.Vector3().crossVectors(contYAxis, contZAxis).normalize()
    const contYOrtho = new THREE.Vector3().crossVectors(contZAxis, contXAxis).normalize()
    const contMat = new THREE.Matrix4().makeBasis(contXAxis, contYOrtho, contZAxis)
    const contQuat = new THREE.Quaternion().setFromRotationMatrix(contMat)

    // world-space desired camera position (continuous)
    const contBaseWorld = contLocal.clone().add(contOutward).applyMatrix4(groupMatrix)
    const cameraOffsetWorld = cameraLocalOffset.clone().applyQuaternion(contQuat)
    ...
    const camZ = contZAxis.clone().multiplyScalar(-1).applyQuaternion(groupQuat).normalize()
    ...
    if (distFromBrick < minDist) {
      const dir =
        fromBrick.length() > 1e-6 ? fromBrick.normalize() : camZ.clone().multiplyScalar(-1)
    ...
    if (forcedBlendRef.current.active && cameraRef.current) {
      ...
      // NORMAL OVERRIDE MOVEMENT PATH (smoothing), but note: camDesiredWorld already computed continuously,
      // so even a small scroll delta will produce a small target delta — camera will be updated every frame.
      if (isOverriding && cameraRef.current) {
        ...
        if (desiredDelta.length() > maxMove) {
          ...
          // lerp ensures per-frame update (small but visible) — no skipping/waiting
