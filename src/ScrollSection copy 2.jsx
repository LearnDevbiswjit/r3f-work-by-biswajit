...
function Scene ({ sheet, guiFadeDefaults = {} }) {
  ...
  useFrame(() => {
    ...
    const snapDurMs =
      (typeof guiFadeDefaults.snapDurationSec === 'number'
        ? guiFadeDefaults.snapDurationSec
        : window._springFadeDefaults?.snapDurationSec || 2) * 1000
  ...
  useFrame((state, delta) => {
    ...
    if (forcedBlendRef.current.active && cameraRef.current) {
      ...
      if (u >= 1) {
        ...
        // --- NEW: write current R3F camera transform into Theatre Camera actor
        try {
          const cam = cameraRef.current
          if (cam && sheet) {
            // try a few common Theatre APIs to set actor state
            let camActor = null
            try {
              // preferred: sheet.object('Camera')
              if (typeof sheet.object === 'function') {
                camActor = sheet.object('Camera')
              }
            } catch (e) {
              camActor = camActor || null
            }
            try {
              // fallback: sheet.get('Camera')
              if (!camActor && typeof sheet.get === 'function') {
                camActor = sheet.get('Camera')
              }
            } catch (e) {
              // ignore
            }
            ...
            const quaternion = { x: cam.quaternion.x, y: cam.quaternion.y, z: cam.quaternion.z, w: cam.quaternion.w }

            if (camActor) {
              // try common setter patterns
              try {
                if (typeof camActor.set === 'function') {
                  camActor.set({ position: pos, quaternion })
                } else if (camActor.state && typeof camActor.state.set === 'function') {
                  camActor.state.set({ position: pos, quaternion })
                } else if (typeof camActor.update === 'function') {
                  camActor.update({ position: pos, quaternion })
                } else {
                  // last resort: mutate props/state directly if available
                  try {
                    camActor.props = camActor.props || {}
                    camActor.props.position = pos
                    camActor.props.quaternion = quaternion
                    if (camActor.state && typeof camActor.state.set === 'function') {
                      camActor.state.set(camActor.props)
                    }
                  } catch (e) {}
                }
              } catch (e) {
                console.warn('[Spring] failed to set theatre camera via common methods', e)
              }
            } else {
              // if no actor handle, try to write into sheet sequence/direct state if possible
              try {
                if (sheet && sheet.sequence && sheet.sequence.pointer) {
                  // best-effort: set global values (may be no-op on some versions)
                  try {
                    if (sheet.state && typeof sheet.state.set === 'function') {
                      sheet.state.set({ Camera: { position: pos, quaternion } })
                    }
                  } catch (e) {}
                }
              } catch (e) {
          ...
          console.warn('[Spring] failed to write camera to theatre actor', e)
        }
        ...
        try {
          // small timeout to allow theatre state to accept writes before resuming
          setTimeout(() => {
            try {
...
{isOverriding ? (   
  ...
  // when not overriding: use theatre-wrapped camera with theatreKey
  <PerspectiveCamera
    ref={cameraRef}
    theatreKey="Camera"
    makeDefault
