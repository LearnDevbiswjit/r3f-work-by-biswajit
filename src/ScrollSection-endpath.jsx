...
function Scene ({ sheet, guiFadeDefaults = {} }) {
  ...
  useFrame(() => {
    ...
    // NOTE: We DO NOT use overrideEndSec anymore.
    // Instead: override ends when SpringPath reaches last brick (detected in render loop).
    // isOverriding state will be updated in the main frame loop based on that.
  })
  ...
  useFrame((state, delta) => {
    ...
    // use a small epsilon so we detect near-end reliably
    const epsilon = 0.0001
    const lastIndex = Math.max(0, count - 1)
    const smoothed = smoothedIndexRef.current || 0
    const reachedEnd = smoothed >= lastIndex - epsilon

    // if reached end and a fade controller session is active -> force exit and cancel forcedBlend immediately
    if (reachedEnd) {
      try {
        const ctrl = window._springFadeController || null
        if (ctrl && ctrl.sessionId && !(ctrl.exit || ctrl.exited)) {
