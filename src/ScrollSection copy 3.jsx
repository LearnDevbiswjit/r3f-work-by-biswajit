...
/* ---------------- small util: export camera pose as JSON and trigger download ---------------- */
function exportCameraPoseToJson (camera, label = 'springpath-last-camera') {
  if (!camera) return
...
function Scene ({ sheet, guiFadeDefaults = {} }) {
  ...
  // track previous sequence seconds to detect *crossing* overrideStartSec
  ...
  useFrame(() => {
    ...
    // Detect crossing of the START threshold:
    const prevSeq = prevSeqPosRef.current || 0
    const crossedStart = prevSeq < AUTOSTART_SEC && seqPosSeconds >= AUTOSTART_SEC

    // If crossed start -> trigger fade according to direction (down -> Enter, up -> Exit)
    if (crossedStart) {
      const defaults =
        (typeof window !== 'undefined' && window._springFadeDefaults) ||
        guiFadeDefaults ||
      ...
      // If we've already triggered for this direction and haven't left below START, skip
      ...
      if (lastTriggeredDirRef.current !== sessionDir) {
        if (allowTrigger) {
          ...
          // compute forcedBlend target (same as before) so camera snaps into position when start crossed
          try {
          ...
          // Build controller object explicitly for START-crossing:
          // - When directionDown -> trigger ENTER only
          // - When directionUp   -> trigger EXIT only
          let controller = {
            sessionId,
            enter: false,
            entered: false,
          ...
          // record that we've triggered this direction until we go back below START
          lastTriggeredDirRef.current = sessionDir
        } else {
          // cooldown blocked; keep defaults
          window._springFadeDefaults = {
            forcedBlendMs: forcedBlendMsVal,
            fadeExitMs: fadeExitMsVal,
            fadeHoldMs: fadeHoldMsVal,
        ...
        // already triggered for this direction and haven't gone below START -> do nothing (but keep defaults)
        window._springFadeDefaults = {
          forcedBlendMs: forcedBlendMsVal,
          fadeExitMs: fadeExitMsVal,
          fadeHoldMs: fadeHoldMsVal,
    ...
    // Reset the lastTriggeredDirRef when we go back below AUTOSTART_SEC so next crossing retriggers
    if (seqPosSeconds < AUTOSTART_SEC) {
    ...
    // keep isOverriding state (for camera toggle)
    // const shouldBeOverriding = seqPosSeconds >= AUTOSTART_SEC && seqPosSeconds < (typeof guiFadeDefaults.overrideEndSec === 'number' ? guiFadeDefaults.overrideEndSec : window._springFadeDefaults?.overrideEndSec || 150)
  const shouldBeOverriding = seqPosSeconds >= AUTOSTART_SEC
    prevOverrideRef.current = shouldBeOverriding
    setIsOverriding(shouldBeOverriding)

    // update lastRawRef for scroll direction detection
  ...
  useFrame((state, delta) => {
    ...
    // ----------------- CAMERA: continuous along path (KEY CHANGE) -----------------
    // compute camera target based on continuous t (so every micro-scroll updates camera target)
    ...
    // compute base local camera attachment point (continuous)
    // reuse same side-offset logic as bricks, but applied continuously
    let sign = 1
    ...
    // build local brick-like basis for continuous point to rotate offset into world
    ...
    // world-space desired camera position (continuous)
    ...
    // compute camera orientation to face along path (approx): use contZAxis as forward (negated)
    ...
    if (forcedBlendRef.current.active && cameraRef.current) {
      ...
      if (u >= 1) {
        ...
        // ensure theatre resumes when forcedBlend finishes (fallback)
        try {
