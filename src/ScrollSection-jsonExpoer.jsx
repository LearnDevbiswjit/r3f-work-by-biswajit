...
// safe theatre camera key (use a stable constant)
const maybeKey = 'Camera'
...
/* ---------------- Theatre store helpers ---------------- */

/**
 * Try to write camera state to Theatre at a given time (seconds).
 * Best-effort: detects whether sheet.sequence.pointer expects frames or seconds,
 * sets the playhead, writes camera props via studio.transaction (preferred),
 * and restores previous playhead position afterwards.
 *
 * Returns true on success, false on fallback (clipboard/download).
 */
async function storeCameraKeyframeAtTime ({ project, sheetName = 'Scene', camera, timeSec = null }) {
  try {
    if (!project) {
      console.warn('[storeCameraKeyframeAtTime] no project provided')
      return false
    }
    if (!camera) {
      console.warn('[storeCameraKeyframeAtTime] no camera provided')
      return false
    }
    ...
    try {
      ...
      console.warn('[storeCameraKeyframeAtTime] project.sheet error', e)
      sheet = null
    }
    if (!sheet) {
      console.warn('[storeCameraKeyframeAtTime] sheet not found:', sheetName)
      return false
    }

    // take snapshot of current sequence position to restore later
    let prevPos = null
    try {
      prevPos = sheet.sequence.position
    } catch (e) {
      prevPos = null
    }

    // compute desired position in timeline units (frames vs seconds)
    let desiredPos = null
    if (timeSec == null) {
      // fallback: write at current position
      desiredPos = sheet.sequence.position || 0
    } else {
      // heuristic detection
      try {
        ...
        if (ptr) {
          ...
          const ptrLen = typeof ptr.length === 'number' ? ptr.length : NaN
          if (isFinite(ptrLen) && ptrLen > 1000) useFrames = true
        }
        if (useFrames) desiredPos = Math.round(timeSec * fps)
        else desiredPos = timeSec
      } catch (e) {
        desiredPos = timeSec
      }
    }

    // attempt: set sequence position then write via studio.transaction into Camera object
    try {
      if (typeof desiredPos !== 'undefined' && desiredPos != null) {
        try {
          sheet.sequence.position = desiredPos
        } catch (e) {
          // ignore
        }
      }

      // create or get camera object with expected props
      const camObj = sheet.object(maybeKey, {
      ...
      // gather camera values
      const pos = { x: camera.position.x, y: camera.position.y, z: camera.position.z }
      const quat = { x: camera.quaternion.x, y: camera.quaternion.y, z: camera.quaternion.z, w: camera.quaternion.w }
      const near = typeof camera.near === 'number' ? camera.near : 0.1
      const far = typeof camera.far === 'number' ? camera.far : 5000
      const fov = typeof camera.fov === 'number' ? camera.fov : 35
      const zoom = typeof camera.zoom === 'number' ? camera.zoom : 1
      const scale = camera.scale ? { x: camera.scale.x, y: camera.scale.y, z: camera.scale.z } : { x: 1, y: 1, z: 1 }

      // use studio.transaction to set props atomically (preferred)
      try {
        studio.transaction(({ set }) => {
          if (camObj.props && camObj.props.position) set(camObj.props.position, pos)
          if (camObj.props && camObj.props.quaternion) set(camObj.props.quaternion, quat)
          if (camObj.props && camObj.props.near) set(camObj.props.near, near)
          if (camObj.props && camObj.props.far) set(camObj.props.far, far)
          if (camObj.props && camObj.props.fov) set(camObj.props.fov, fov)
          if (camObj.props && camObj.props.zoom) set(camObj.props.zoom, zoom)
          if (camObj.props && camObj.props.scale) set(camObj.props.scale, scale)
        })
        // restore prev pos
        try {
          if (prevPos != null) sheet.sequence.position = prevPos
        } catch (e) {}
        console.log('[storeCameraKeyframeAtTime] saved via studio.transaction at', desiredPos)
        return true
      } catch (err) {
        console.warn('[storeCameraKeyframeAtTime] studio.transaction failed', err)
      }

      // fallback: attempt direct prop writes
      try {
        ...
        console.log('[storeCameraKeyframeAtTime] saved via fallback direct-prop write at', desiredPos)
        return true
      } catch (err2) {
        console.warn('[storeCameraKeyframeAtTime] fallback direct write failed', err2)
      }

    } catch (e) {
      console.warn('[storeCameraKeyframeAtTime] main attempt failed', e)
    }

    // final fallback: copy JSON to clipboard & download file for manual import
    try {
      const payload = {
        timestamp: Date.now(),
        camera: {
      ...
      a.href = url
      a.download = `camera_pose_fallback_${Date.now()}.json`
      document.body.appendChild(a)
      a.click()
      a.remove()
      URL.revokeObjectURL(url)
      console.warn('[storeCameraKeyframeAtTime] fallback: pose copied & downloaded')
    } catch (e) {
      console.warn('[storeCameraKeyframeAtTime] fallback final step failed', e)
    }

    try {
      if (sheet && prevPos != null) sheet.sequence.position = prevPos
    } catch (e) {}

    return false
  } catch (finalErr) {
    console.error('[storeCameraKeyframeAtTime] unexpected', finalErr)
    return false
  }
}
...
export default function ScrollSection () {
  ...
  const { autoSaveKeyframe } = useControls('Theatre Save', {
    autoSaveKeyframe: { label: 'Auto-save final camera to Theatre when forcedBlend completes', value: true }
  })
  ...
  // NEW: Snap enable toggle
  const { snapEnabled } = useControls('Snap', {
    snapEnabled: { label: 'Enable snap freeze at end of forcedBlend', value: true }
  })
...
function ManualSaveButton ({ project }) {
  ...
  function onClickSave () {
    try {
      ...
      if (c) {
        console.info('[ManualSave] camera values:', { pos: c.position.clone(), quat: c.quaternion.clone(), near: c.near, far: c.far, fov: c.fov, zoom: c.zoom })
      }
      // Save at overrideEnd + snapDuration by default (if available)
      const defaults = window._springFadeDefaults || {}
      const tEnd = Number(defaults.overrideEndSec || 240)
      const snap = Number(defaults.snapDurationSec || 2)
      const targetTime = tEnd + snap
      storeCameraKeyframeAtTime({ project, sheetName: 'Scene', camera: camRef.current, timeSec: targetTime }).then(ok => {
        console.log('[ManualSave] store result', ok)
      })
    } catch (e) {
...
function Scene ({ sheet, project, guiFadeDefaults = {} }) {
  ...
  // expose cameraRef for manual save
  useEffect(() => {
  ...
  const forcedBlendRef = useRef({
    active: false,
    startTime: 0,
    duration: (guiFadeDefaults.forcedBlendMs) || DEFAULT_FORCED_BLEND_MS,
          ...
          if (scrollDelta <= STABLE_THRESHOLD) {
            try {
              ...
              if (cameraRef && cameraRef.current) {
                ...
                // ensure R3F uses this camera and pause Theatre defensively
                try {
                  set({ camera: cameraRef.current })
                } catch (e) {}
                try { if (sheet && sheet.sequence) sheet.sequence.pause() } catch (e) {}
                try {
                  if (cameraRef.current) {
                    cameraRef.current.matrixAutoUpdate = true
  ...
  useFrame((state, delta) => {
    ...
    // snap freeze (only used if snapEnabled true)
    if (snapRef.current && snapRef.current.active) {
      ...
      if (elapsed <= (snapRef.current.duration || 0)) {
        ...
        // snap ended: deactivate and possibly save to theatre (if not already saved)
        snapRef.current.active = false
        try {
          const ctrl = window._springFadeController || null
          if (ctrl && ctrl.exit) {
            // resume theatre playback
            try { sheet.sequence.play() } catch (e) {}
          }
        } catch (e) {}
        // If we haven't saved yet and autoSave is enabled, store the keyframe now (this corresponds to overrideEnd + snap)
        try {
          const shouldAuto = guiFadeDefaults.autoSaveKeyframe ?? window._springFadeDefaults?.autoSaveKeyframe ?? true
          if (shouldAuto && !forcedBlendRef.current._savedToTheatre) {
            const targetTime = Number(guiFadeDefaults.overrideEndSec || window._springFadeDefaults?.overrideEndSec || 240) + Number(guiFadeDefaults.snapDurationSec || window._springFadeDefaults?.snapDurationSec || 2)
            storeCameraKeyframeAtTime({ project, sheetName: 'Scene', camera: cameraRef.current, timeSec: targetTime }).then(ok => {
              forcedBlendRef.current._savedToTheatre = ok
              console.log('[snap] store result:', ok, 'at', targetTime)
            })
          }
        } catch (e) {
          console.warn('[snap] post-snap save failed', e)
        }
      }
    }
    ...
    if (forcedBlendRef.current.active && cameraRef.current) {
      ...
      if (u >= 1) {
        ...
        // At forcedBlend completion: either snap (freeze) OR immediately save to Theatre (if snap disabled)
        try {
          const shouldSnap = (typeof guiFadeDefaults.snapEnabled === 'boolean' ? guiFadeDefaults.snapEnabled : window._springFadeDefaults?.snapEnabled ?? true)
          if (shouldSnap) {
            // activate snap freeze
            snapRef.current.active = true
            snapRef.current.start = performance.now()
            snapRef.current.duration = (guiFadeDefaults.snapDurationSec || window._springFadeDefaults?.snapDurationSec || 2) * 1000
            snapRef.current.pos = cameraRef.current.position.clone()
            snapRef.current.quat = cameraRef.current.quaternion.clone()
            snapRef.current.sessionId = fb.sessionId
            // theatre resume & save handled after snap end (in earlier code)
          } else {
            // no snap: directly save to Theatre (if autoSave enabled) and resume theatre immediately
            try {
              if (guiFadeDefaults && guiFadeDefaults.autoSaveKeyframe && project) {
                // compute target time (overrideEnd + 0 since no snap)
                const targetTime = Number(guiFadeDefaults.overrideEndSec || window._springFadeDefaults?.overrideEndSec || 240)
                storeCameraKeyframeAtTime({ project, sheetName: 'Scene', camera: cameraRef.current, timeSec: targetTime }).then(ok => {
                  forcedBlendRef.current._savedToTheatre = ok
                  console.log('[forcedBlend->direct-save] result', ok, 'at', targetTime)
                })
              }
            } catch (e) {
          ...
          console.warn('[ForcedBlend] completion handling failed', e)
        }
  ...
  return (
    <>
      {/* Single camera instance only (theatre editable) */}
      <PerspectiveCamera
        ref={cameraRef}
        theatreKey={maybeKey}
        makeDefault
