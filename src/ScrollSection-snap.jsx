...
export default function ScrollSection () {
  ...
  const {
    ...
    snapDurationSec: {
      value: 10, // default 10s per user's request
      min: 0.1,
      max: 60,
      step: 0.1,
...
function Scene ({ sheet, guiFadeDefaults = {} }) {
  ...
  const snapRef = useRef({
    ...
    start: 0,
    duration:
      (typeof guiFadeDefaults.snapDurationSec === 'number'
        ? guiFadeDefaults.snapDurationSec
        : 10) * 1000,
    pos: new THREE.Vector3(),
    quat: new THREE.Quaternion(),
    sessionId: null,
    storedToTheatre: false
  })
  ...
  useFrame(() => {
    ...
    const AUTOEND_SEC =
      typeof guiFadeDefaults.overrideEndSec === 'number'
        ? guiFadeDefaults.overrideEndSec
        : window._springFadeDefaults?.overrideEndSec || 150
    // snap duration read from GUI/defaults but we will enforce user-set value
    const snapDurMs =
      (typeof guiFadeDefaults.snapDurationSec === 'number'
        ? guiFadeDefaults.snapDurationSec
        : window._springFadeDefaults?.snapDurationSec || 10) * 1000
      ...
      if (shouldOverrideNow) {
        ...
        try {
          ...
          if (cameraRef && cameraRef.current) {
            ...
            // --- ENSURE R3F uses this camera immediately and pause Theatre safely ---
            try {
              // set active camera in R3F
              set({ camera: cameraRef.current })
              // pause theatre (defensive)
              if (sheet && sheet.sequence) sheet.sequence.pause()
        ...
        // ensure R3F camera remains consistent after exit
        try {
          set({ camera: cameraRef.current })
        } catch (e) {}
      }
    ...
    // --- If we are past AUTOEND_SEC, ensure snap + resume logic (create snap ONCE) ---
    if (seqPosSeconds >= AUTOEND_SEC) {
      try {
        // only create snap once
        if (snapRef.current && !snapRef.current.active) {
          // capture current camera pose (if available)
          if (cameraRef && cameraRef.current) {
            snapRef.current.active = true
            snapRef.current.start = performance.now()
            // respect GUI but default to 10s; enforce min 0.1s
            const guiSnap = typeof guiFadeDefaults.snapDurationSec === 'number'
              ? guiFadeDefaults.snapDurationSec
              : window._springFadeDefaults?.snapDurationSec || snapDurationSec || 10
            snapRef.current.duration = Math.max(100, guiSnap * 1000)
            snapRef.current.pos.copy(cameraRef.current.position)
            snapRef.current.quat.copy(cameraRef.current.quaternion)
            snapRef.current.sessionId = makeSessionId()
            snapRef.current.storedToTheatre = false

            // Pause theatre while snap is active (we want snap to hold)
            try {
              if (sheet && sheet.sequence) sheet.sequence.pause()
            } catch (e) {}

            // ensure r3f uses our camera instance during snap
            try {
              set({ camera: cameraRef.current })
              if (cameraRef.current) {
                cameraRef.current.matrixAutoUpdate = true
            ...
            // attempt to store a keyframe immediately (best-effort)
            try {
              const proj = window.__THEATRE_PROJECT__ || (typeof getProject !== 'undefined' && getProject('myProject'))
              const sheetLocal = (proj && typeof proj.sheet === 'function') ? proj.sheet('Scene') : sheet
              if (sheetLocal && typeof sheetLocal.object === 'function') {
                const camObj = sheetLocal.object && sheetLocal.object('Camera')
                if (camObj && typeof camObj.set === 'function') {
                  camObj.set({
                    position: [cameraRef.current.position.x, cameraRef.current.position.y, cameraRef.current.position.z],
                    quaternion: [cameraRef.current.quaternion.x, cameraRef.current.quaternion.y, cameraRef.current.quaternion.z, cameraRef.current.quaternion.w]
                  })
                  snapRef.current.storedToTheatre = true
                  console.info('[Spring] theatre: stored Camera snapshot as keyframe at snap START (best-effort).')
                }
              }
            } catch (e) {
              console.warn('[Spring] theatre keyframe attempt at snap START failed', e)
            }
          } else {
            // still create a time-based snap (no camera)
            snapRef.current.active = true
            snapRef.current.start = performance.now()
            snapRef.current.duration = Math.max(100, snapDurMs)
            snapRef.current.sessionId = makeSessionId()
            snapRef.current.storedToTheatre = false
          }
        }

        // ensure sheet is paused (safety)
        try {
          if (sheet && sheet.sequence) sheet.sequence.pause()
        } catch (e) {}
      } catch (e) {}
  ...
  useFrame((state, delta) => {
    ...
    if (snapRef.current && snapRef.current.active) {
      ...
      if (elapsed <= (snapRef.current.duration || 0)) {
        ...
        // Attempt to store a reliable keyframe at snap END if not already stored
        try {
          if (!snapRef.current.storedToTheatre && cameraRef && cameraRef.current) {
            const proj = window.__THEATRE_PROJECT__ || (typeof getProject !== 'undefined' && getProject('myProject'))
            const sheetLocal = (proj && typeof proj.sheet === 'function') ? proj.sheet('Scene') : sheet
            if (sheetLocal && typeof sheetLocal.object === 'function') {
              const camObj = sheetLocal.object && sheetLocal.object('Camera')
              if (camObj && typeof camObj.set === 'function') {
                ...
                console.info('[Spring] theatre: stored Camera snapshot as keyframe at snap END (best-effort).')
              }
            }
          }
        } catch (e) {
          console.warn('[Spring] theatre keyframe attempt at snap END failed', e)
        }

        // Resume theatre playback and re-enable theatre camera binding
        try {
          if (sheet && sheet.sequence) sheet.sequence.play()
        } catch (e) {}

        try {
          // rebind r3f/theatre camera so Theatre regains control (isOverriding false will make theatreKey back)
          set({ camera: cameraRef.current })
        } catch (e) {}

        // clear any suppression flags so other logic can resume normally
        window._springSuppressTheatreResume = false

        // ensure override flags reflect exit
        prevOverrideRef.current = false
        setIsOverriding(false)

        // Do not return here; we let follow-path logic run next frames normally
      }
    }
    ...
    if (forcedBlendRef.current.active && cameraRef.current) {
      ...
      if (u >= 1) {
        ...
        // ensure r3f camera consistency after forcedBlend finishes
        try {
          set({ camera: cameraRef.current })
        } catch (e) {}
      }
          ...
          if (ctrl && ctrl.sessionId && !(ctrl.exit || ctrl.exited)) {
            if (ctrl.enter) {
              ...
              if (exitAllowed) {
                ...
                // ensure r3f camera still consistent
                try {
                  set({ camera: cameraRef.current })
                } catch (e) {}
              } else {
  ...
  return (
    <>
      {/* single camera instance only — control behaviour imperatively in useFrame
          theatreKey is toggled based on isOverriding so Theatre won't overwrite during overrides */}
{isOverriding ? (
  <perspectiveCamera
    ref={cameraRef}
    makeDefault
